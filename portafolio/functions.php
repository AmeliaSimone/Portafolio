...
function portafolio_setup() {
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'portafolio_custom_background_args',
			array(
...
function portafolio_scripts() {
	wp_enqueue_style( 'portafolio-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'portafolio-style', 'rtl', 'replace' );

	wp_enqueue_script( 'portafolio-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
...
function portafolio(){
    ...
    wp_register_style('bootstrap','https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css', 'all');
      ...
      //wp_register_style('portafolio_estilos', get_template_directory_uri() . '/assets/css/miestilo.css', 'all');
    ...
    add_action('wp_enqueue_scripts', 'portafolio');

function fonts() {
    
     wp_register_style( 'fuentes', 'https://fonts.googleapis.com/css2?family=Itim&family=Yomogi&display=swap', array(), null );
...
function new_jquery() {
	// Encola la versión de jQuery incluida en WordPress
	wp_enqueue_script( 'jquery' );
  }
  
  add_action( 'wp_enqueue_scripts', 'new_jquery' );
  

function scripts() {
	wp_register_script('bootstrap','https://stackpath.bootstrapcdn.com/bootstrap/4.5.0/js/bootstrap.min.js', array('jquery'), '4.5.0', true);
